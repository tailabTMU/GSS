...
def main(args):
    ...
    test_true_classes = {}
    avg_train_time = {}
    for train_index, _ in skf.split(iid_data, labels):
        split_number += 1
        ...
        train_dataset_idx = iid_data[train_index]
        ...
        root_ckpt_dir = pjoin(cwd, 'saved_models', 'saved_models_enzymes_all_data', f'{config["L"]}_layers',
                              f'{config["hidden_dim"]}_hidden_size')
        os.makedirs(root_ckpt_dir, exist_ok=True)
        results_dir = pjoin(cwd, 'results', 'results_enzymes_all_data', f'{config["L"]}_layers',
                            f'{config["hidden_dim"]}_hidden_size')
        ...
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                # Saving checkpoint
                ckpt_dir = os.path.join(root_ckpt_dir, f"RUN_" + str(split_number))
        ...
        with open(pjoin(results_dir,
                        f'split_{split_number}_{model_name}_test_logit_vals.txt'),
                  'w+') as myfile:
            np.savetxt(myfile, logit_vals)

        with open(
                pjoin(results_dir,
                      f'split_{split_number}_{model_name}_test_true_classes.txt'),
                'w+') as myfile:
            np.savetxt(myfile, true_classes)

        with open(pjoin(results_dir,
                        f'split_{split_number}_{model_name}_test_predicted_classes.txt'),
                  'w+') as myfile:
            np.savetxt(myfile, predicted_classes)

        with open(pjoin(results_dir,
                        f'split_{split_number}_{model_name}_time_to_train.txt'),
                  'w+') as myfile:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Training a Graph Classifiers with CrossEntropy Loss using the Enzymes Dataset')
