    ...
    for iter_num in range(1, 6):
        ...
        for split_number in range(1, 6):
            ...
            features_file = pjoin(results_dir,
                                  f'{model_name}_layer_{num_layers}_iter_{iter_num}_split_{split_number}_test_features.txt')
        ...
        iter_data.to_csv(pjoin(results_dir, f'patient_features_iter_{iter_num}.csv'), index=False)
        ...
        for n_clusters in [5, 10, 15, 20]:
            ...
            clusters_df.to_csv(pjoin(results_dir, f'k_{n_clusters}', f'patient_cluster_iter_{iter_num}.csv'),
                               index=False)
            kmeans_dir = pjoin(cwd, 'saved_models', f'k_means{suffix}', f'k_{n_clusters}')
            os.makedirs(kmeans_dir, exist_ok=True)
            dump(km_clustering_model, pjoin(kmeans_dir, f'k_means_model_iter_{iter_num}.pkl'))
            print()
