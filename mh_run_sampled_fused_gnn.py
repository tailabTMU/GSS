    ...
    for n_clusters in [5, 10, 15, 20]:
        ...
        for iter_num in range(1, num_iters + 1):
            ...
            loss_checkpoint_path = pjoin(cwd, 'saved_models', f'saved_models{suffix}',
                                         ...
                                         f'{hidden_size}_hidden_size',
                                         f'k_{n_clusters}',
                                         f'iter_{iter_num}_{model_name}.pt')
            final_checkpoint_path = pjoin(cwd, 'saved_models', f'saved_models{suffix}',
                                          f'{num_layers}_layers',
                                          f'{hidden_size}_hidden_size',
                                          f'k_{n_clusters}',
                                          f'iter_{iter_num}_{model_name}_final.pt')
            ...
            epoch_loss_acc_file = pjoin(cwd, 'results', f'results{suffix}', f'{num_layers}_layers',
                                        f'{hidden_size}_hidden_size',
                                        f'k_{n_clusters}',
                                        f'iter_{iter_num}_{model_name}_epoch_loss_acc.txt')
            ...
            for res_type in ['loss', 'final']:
                ...
                info_file = pjoin(cwd, 'results', f'results{suffix}', f'{num_layers}_layers',
                                  f'{hidden_size}_hidden_size',
                                  f'k_{n_clusters}',
                                  f'iter_{iter_num}_{model_name}_stop_on_{res_type}_info.txt')
                ...
                with open(pjoin(cwd, 'results', f'results{suffix}', f'{num_layers}_layers',
                                f'{hidden_size}_hidden_size', f'k_{n_clusters}',
                                f'train_actual_iter_{iter_num}_{model_name}_stop_on_{res_type}.txt'),
                          'w+') as myfile:
                    np.savetxt(myfile,
                               train_actual_classes_model_1)
                with open(pjoin(cwd, 'results', f'results{suffix}', f'{num_layers}_layers',
                                f'{hidden_size}_hidden_size', f'k_{n_clusters}',
                                f'val_actual_iter_{iter_num}_{model_name}_stop_on_{res_type}.txt'),
                          'w+') as myfile:
                    np.savetxt(myfile,
                ...
                with open(pjoin(cwd, 'results', f'results{suffix}', f'{num_layers}_layers',
                                f'{hidden_size}_hidden_size', f'k_{n_clusters}',
                                f'train_pred_iter_{iter_num}_{model_name}_stop_on_{res_type}.txt'),
                          'w+') as myfile:
                    np.savetxt(myfile,
                               train_predicted_classes_model_1)
                with open(pjoin(cwd, 'results', f'results{suffix}', f'{num_layers}_layers',
                                f'{hidden_size}_hidden_size', f'k_{n_clusters}',
                                f'val_pred_iter_{iter_num}_{model_name}_stop_on_{res_type}.txt'),
                          'w+') as myfile:
                    np.savetxt(myfile,
