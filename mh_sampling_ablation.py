...
if __name__ == '__main__':
    ...
    for iter_num in range(1, 6):
        ...
        for n_clusters in [5, 10, 15, 20]:
            ...
            for idx, group_proportions in enumerate(group_portions_ablation):
                ...
                X_train = X_train.astype(int).tolist()
                np.savetxt(pjoin(data_dir, f'k_{n_clusters}', f'train_ids_iter_{iter_num}_ab_{idx}.txt'), X_train, fmt='%s')
                X_val = X_val.astype(int).tolist()
                np.savetxt(pjoin(data_dir, f'k_{n_clusters}', f'val_ids_iter_{iter_num}_ab_{idx}.txt'), X_val, fmt='%s')
