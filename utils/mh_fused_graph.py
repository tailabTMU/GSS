...
import torch
from utils.mh_fused_dataset import read_fused_data
import pandas as pd
import numpy as np
...
class MHFusedGraphDataset(Dataset):
    ...
    def base_directory(self) -> str:
        return osp.join(self.raw_dir, 'large_fused_graph')

    @property
    def processed_dir(self) -> str:
        return osp.join(self.base_directory, f'processed_data')
    ...
    def process(self):
        read_fused_data(osp.join(self.raw_dir), base_directory=self.base_directory,
    ...
    def get(self, idx):
        ...
        for node_type in node_types:
            data[node_type, f'{node_type}_{node_type}', node_type].edge_index = torch.tensor([[], []],
                                                                                             dtype=torch.long)
            data[node_type, f'{node_type}_{node_type}', node_type].edge_attr = np.array([])
        return data

    def __repr__(self) -> str:
        return f'MHFusedGraphDataset()'
